...
public class Utils {
	private static DoubleTapReloadRecognizer mDoubleTapReloadRecognizer = new DoubleTapReloadRecognizer();
	...
	public static boolean onKeyUp(Activity activity,ReactNativeHost host,int keyCode){
		if (host.hasInstance() && host.getUseDeveloperSupport()) {
			...
			boolean didDoubleTapR = mDoubleTapReloadRecognizer.didDoubleTapR(keyCode, activity.getCurrentFocus());
			if (didDoubleTapR) {
				host.getReactInstanceManager()
